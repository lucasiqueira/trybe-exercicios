...
//  Resposta: .tech {transform: translateY(-20px);} do arquivo CSS.
...
const changeTechBox = (event) => {
  const techElements = document.getElementsByClassName('tech');
  techElements[0].innerHTML = event.target.value;
}

input.addEventListener('input', changeTechBox);
